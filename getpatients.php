...
<html>
    ...
    <table>
        <?php
            ...
            $query = 'SELECT * FROM Patient, Vaccination, Vaccine WHERE Patient.OHIP=Vaccination.patientOHIP AND Patient.OHIP="' . $whichPatient . '" AND Vaccine.lot=Vaccination.vaccineLot';
            $result=$connection->query($query);

            if (!$row=$result->fetch()){
                echo "No patient under that OHIP Number";
            } else {
                echo "
                <tr>
